...
Playing with the market can be risky, and the complexity ceiling for a truly _good_ implementation is high. But that's not why we're here today, so we must resist that urge. Instead, we'll check two simple rules:
